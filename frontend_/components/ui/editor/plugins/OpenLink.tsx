...
export function OpenLinkPlugin() {
  ...
  return (
    <div
      ...
      className={`absolute flex items-center justify-between bg-slate-100 border-[1px] border-slate-300 rounded-md p-1 gap-[2px] ${pos?.x && pos.y ? "opacity-1 visible" : "opacity-0 invisible"
        }`}
    >
      {link && !copied ? (
        <a
