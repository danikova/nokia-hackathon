...
func init() {
	m.Register(func(db dbx.Builder) error {
		...
		json.Unmarshal([]byte(`{
			"query": "SELECT\n    run_results.workspace AS id,\n    AVG(run_results.execution_time) AS average_execution_time,\n    AVG(LENGTH(run_results.output)) AS average_output_length,\n    COUNT(DISTINCT(run_results.run_id)) AS number_of_runs,\n    SUM(CASE run_results.returncode=0 WHEN 1 THEN 0 END) as number_of_green_runs\nFROM run_results\nGROUP BY run_results.workspace"
		}`), &options)
		collection.SetOptions(options)

		// remove
		collection.Schema.RemoveField("ntjp4fe6")

		// remove
		collection.Schema.RemoveField("iftpkhrp")

		// remove
		collection.Schema.RemoveField("zrtzwrbh")

		// remove
		collection.Schema.RemoveField("lpgmklps")
		...
		options := map[string]any{}
		json.Unmarshal([]byte(`{
			"query": "SELECT\n    run_results.workspace AS id,\n    AVG(run_results.execution_time) AS average_execution_time,\n    AVG(LENGTH(run_results.output)) AS average_output_length,\n    COUNT(DISTINCT(run_results.run_id)) AS number_of_runs,\n    SUM(CASE run_results.is_success=TRUE WHEN 1 THEN 0 END) as number_of_green_runs\nFROM run_results\nGROUP BY run_results.workspace"
		}`), &options)
		collection.SetOptions(options)
		...
		// remove
		collection.Schema.RemoveField("hijh5tcy")

		// remove
		collection.Schema.RemoveField("kfzcfzes")

		// remove
		collection.Schema.RemoveField("qbwaa2rr")

		// remove
		collection.Schema.RemoveField("xiw49qxx")
